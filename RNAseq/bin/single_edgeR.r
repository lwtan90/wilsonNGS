...
names(filelist)=c("filename","sample")

grouping = read.table("list.txt")
names(grouping)=c("sample","group")

grouping = grouping[ grouping$group %in% c(args[1],args[2]), ]
grouping$group = factor(grouping$group,levels=c(group1,group2))
grouping$color = as.character(rep("blue"))
grouping$color[ grouping$group==group2 ] = "red"

filelist = filelist[ filelist$sample %in% grouping$sample,]
filelist = filelist[match(grouping$sample, filelist$sample),]


data = readDGE(file=filelist$filename,group=grouping$group,labels=grouping$sample)
...
genelength = genelength[match(rownames(filtered),genelength$id), ]


set = newSeqExpressionSet(as.matrix(filtered),phenoData=data.frame(grouping$group,row.names=grouping$sample))
...
design = model.matrix(~0+grouping$group,data=pData(set))

y = DGEList(counts=counts(set),group=grouping$group)
...
pcaplot_global = paste("globalpca_",group1,"_",group2,".pdf",sep="")
clusterplot_global = paste("globalclustering_",group1,"_",group2,".pdf",sep="")
pcaplot_degene = paste("degenepca_",group1,"_",group2,".pdf",sep="")
heatmapdegene = paste("degene_heatmap_",group1,"_",group2,".pdf",sep="")
scatterplot_notext = paste("scatterplot_notext_",group1,"_",group2,".pdf",sep="")
volcanoplot_notext = paste("volcanoplot_notext_",group1,"_",group2,".pdf",sep="")
scatterplot_withtext = paste("scatterplot_withtext_",group1,"_",group2,".pdf",sep="")
volcanoplot_withtext = paste("volcanoplot_withtext_",group1,"_",group2,".pdf",sep="")




control = fpkm[,colnames(fpkm) %in% grouping$sample[ grouping$group==group1] ]
condition2 = fpkm[,colnames(fpkm) %in% grouping$sample[ grouping$group==group2] ]
...
pdf(clusterplot_global,width=5,height=3)
plot(hc)
dev.off()
...
pdf(pcaplot_degene,width=3.5,height=3)
...
grouping = grouping[match(colnames(fpkm),grouping$sample),]
...
pdf(heatmapdegene,width=4,height=7)
heatmap.2(as.matrix(fpkm.sig),Colv=as.dendrogram(hc),Rowv=as.dendrogram(hr),scale="none",breaks=c(seq(-2,2,by=0.04)),col=colorRampPalette(c("blue","white","orange"))(100),density.info="none",trace="none",ColSideColors=as.character(grouping$color))
dev.off()
...
head(sigcondition2)
head(statistics)
pdf(scatterplot_notext,width=5,height=5)
...
pdf(scatterplot_withtext,width=10,height=10)
...
pdf(volcanoplot_notext,width=5,height=5)
...
pdf(volcanoplot_withtext,width=10,height=10)
